...
| **Recalibrate** | `recalibrate_mimic_icu` / `recalibrate_to_network_median` (ICU) / `recalibrate_to_full_hospital` (whole hospital) | Reshapes acuity/LOS/rates to the target population; adds length-aware generator paths and terminal deterioration. MIMIC ICU samples use `recalibrate_mimic_icu` (preserves fitted MIMIC blocks; gated NIV; RF phenotypes; ADT arrivals). Network-median presets still use `recalibrate_to_network_median` on `base_pack/`. Full-hospital is ward-dominant and couples spine `admission_route` to ADT arrival. Deep-copies the input pack. | Documented parameters; no randomness. |
...
**The ICU sample** (`sample_dataset/`, ~5k encounters) — from `mimic_all28` +
`recalibrate_mimic_icu` (see `sample_dataset/spec.toml`):
...
**Per-table catalog** (fitted / prior / derived + MIMIC realism notes):
